...
def calculate_fov_angles(w, h, h_fov_deg, force_aspect=None):
    # Use 4:3 ratio to ensure images look like proper photos, not stretched strips
    if force_aspect:
        aspect = force_aspect
    else:
        aspect = w / h
...
def rotate_coords(x, y, yaw_deg):
    # --- FIX: STANDARD ROTATION MATRIX ---
    # This guarantees the shape remains a rectangle.
    # The previous code used a swapped formula which caused the "Skew".
...
def project_ray(lat_origin, lon_origin, alt, total_r, total_p, total_y, w, h, fov, force_aspect=None):
    ...
    # Projection (Flat Earth approximation for small footprint)
...
def run(metadata_path, image_dir, output_dir, cam_pitch, cam_yaw, h_fov=82.0):
    ...
    # Sort to ensure logical processing order
    df = df.sort_values('filename')
    
    # FIX: Force 4:3 Aspect Ratio (1.3333)
    FORCED_ASPECT = 1.3333 
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Georeferencing", unit="img"):
        ...
        try:
            ...
            # To get uniform tiles like your "Desired Result" image:
            # 1. Ignore Drone Pitch (The Gimbal compensates for this).
            # 2. Ignore Drone Roll.
            # 3. Only use Drone Yaw + Camera Mounting Angle.
            t_pitch = cam_pitch 
            t_roll = 0.0        
    ...
    # Save Verification CSV for Analysis
